...
#define M3508_POS2SPEED_SCALE  19.2f   // 位置环输出 → 速度环目标 RPM 换算系数

static volatile uint8_t m3508_fb_ready = 0;   // 是否已收到过至少一帧 0x201 真实反馈
...
HAL_StatusTypeDef M3508_ReadStatus(M3508_CAN_All *m3508_can)
{
    ...
    for (uint8_t i = 0; i < 8; i++)
    {
        ...
        result = HAL_OK;
        m3508_fb_ready = 1;   // 已收到至少一帧真实反馈（供上层判断何时可安全吸附位置目标）
    }

    return result;
}

// 反馈是否已就绪（收到过至少一帧 0x201）
uint8_t M3508_IsFeedbackReady(void) {
    return m3508_fb_ready;
}
    ...
    for (int i = 0; i < 8; i++) {
        ...
        switch (motor->pid_mode) {
            ...
            case M3508_POSITIONPID_MODE:  // 位置环模式
                ...
                {
                	double perr = motor->position_pid.target - motor->position_pid.current;
                	if (perr >  4096) perr -= 8192;
                	if (perr < -4096) perr += 8192;
                	if (perr < 3.0 && perr > -3.0) {
                	    output = 0.0;
                	}
                }
            ...
            case M3508_CASCADE_MODE:  // 串级模式
                ...
                output = PID_Loop_Compute(&motor->position_pid, 0, M3508_ENCODER_RESOLUTION);
                /* 位置死区：误差 <5 计数（约0.22°）时速度目标置 0。
                   否则静止/低速时编码器 ±1~2 计数抖动经 ×19.2 放大成剧烈抖动 */
                {
                    double perr = motor->position_pid.target - motor->position_pid.current;
                    if (perr >  4096) perr -= 8192;   /* 处理 0/8191 边界 */
                    if (perr < -4096) perr += 8192;
                    if (perr < 2.0 && perr > -2.0) {
                        output = 0.0;
                    }
                }
