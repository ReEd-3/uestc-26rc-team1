...
void M3508_PID_Update(M3508_CAN_All *m3508_can) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        output += motor->current_feedforward;

        /* 限幅到电流范围 [-16384, 16384] */
        if (output > M3508_CURRENT_MAX)
        		output = M3508_CURRENT_MAX;
        if (output < M3508_CURRENT_MIN)
        		output = M3508_CURRENT_MIN;
...
void M3508_SetCurrentFeedforward(M3508_HandleTypeDef *motor, int16_t ff_value) {
    if (motor != NULL)
    {
        motor->current_feedforward = ff_value;
    }
}
